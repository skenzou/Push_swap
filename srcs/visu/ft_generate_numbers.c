...
/*   Updated: 2019/06/15 15:26:49 by midrissi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			ft_clear_data(t_visu *visu)
{
	int i;
...
static void			ft_reload_visu(t_visu *visu)
{
	int			stdout;
	int			fd;
	t_list		*instructions;

	visu->stack_size = ft_read_from_file(VALUES, &visu->stack_a);
	...
	if (dup2(stdout, STDOUT_FILENO) == -1 ||
							ft_read_instructions(&instructions, INSTRUCTIONS))
	{
		ft_free_visu(visu);
		exit(1);
	}
